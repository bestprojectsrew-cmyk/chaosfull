...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check active games first — if handled, skip normal AI response
    if is_group:
        handled = await handle_game_messages(update, context)
        if handled:
            return
    group_context_block = ""
    jump_reason = "private"
    ...
    if is_group:
        ...
        respond, jump_reason = should_jump_in(
            text, update.effective_chat.id, is_mention, is_reply
        )
        if not respond:
            return
...
async def set_bot_commands(app: Application):
    ...
    await app.bot.set_my_commands([
        ...
        BotCommand("memory",      "what you know about me"),
        BotCommand("clearmemory", "wipe your memory of me"),
    ...
    await app.bot.set_my_commands([
        ...
        BotCommand("warn",   "warn a user (3 = ban)"),
        BotCommand("warns",  "check warn count"),
        BotCommand("pin",    "pin a message"),
        BotCommand("couple", "couple of the day 💑"),
        BotCommand("game",   "start a group game 🎮"),
